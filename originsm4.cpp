...
static unsigned long sm4Lt(unsigned long ka)
{
	...
	unsigned char b[4];
    PUT_ULONG_BE(ka,a,0)
    std::cout<<"before sbox:"<<std::hex<<(int)a[0]<<":"<<(int)a[1]<<":"<<(int)a[2]<<":"<<(int)a[3]<<std::endl;
    b[0] = sm4Sbox(a[0]);
    b[1] = sm4Sbox(a[1]);
    b[2] = sm4Sbox(a[2]);
    b[3] = sm4Sbox(a[3]);
	GET_ULONG_BE(bb,b,0)
    std::cout<<"sbox:"<<std::hex<<(int)b[0]<<(int)b[1]<<(int)b[2]<<(int)b[3]<<std::endl;
...
{
    std::cout<<"x1^x2^x3^rk:"<<std::hex<<(x1^x2^x3^rk)<<std::endl;
